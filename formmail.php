...
$recipient = "contacto@originhq.cl";
...
// reside on.
$referers = array ('www.originhq.cl', 'originhq.cl');
...
// send it off Para register_globals en off
mail_it(stripslashes($content), isset($_POST['subject'])?stripslashes($_POST['subject']):"Origin HQ Contacto Pag Web", $_POST['email'], $recipient);
if (file_exists($ar_file)) {
   $fd = fopen($ar_file, "rb");
   $ar_message = fread($fd, filesize($ar_file));
   fclose($fd);
   mail_it($ar_message, ($ar_subject)?stripslashes($ar_subject):"RE:Origin HQ Contacto Pag Web", ($ar_from)?$ar_from:$recipient, $email);
}

// if the redirect option is set: redirect them. Para register_globals en off
if (isset($_POST['redirect'])) {
   header("Location: ". $_POST['redirect']);
   exit;
} else {
   echo "Gracias por escribirnos en breve nos pondremos en contacto con usted\n";
   echo "<br><br>\n";
